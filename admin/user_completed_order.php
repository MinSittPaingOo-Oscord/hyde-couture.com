...
$query = "SELECT * FROM orderr JOIN account ON orderr.accountID = account.accountID WHERE orderr.orderStatus = 2 $whereDate AND orderr.accountID = $userID ORDER BY orderr.orderDate DESC";
...
if ($export === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=completed_orders_' . date('Ymd_His') . '.csv');
...
<body>
    <?php
        ...
        echo "<h1>User Completed Orders</h1>";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            echo "<div class='no-orders'>No completed orders at the moment.</div>";
        }
