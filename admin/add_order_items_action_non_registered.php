...
if ($quantity <= 0) {
    header("Location: specific_manual_order_non_registered.php?orderID=$orderID&error=invalid_quantity");
    exit;
}
...
if (!$row) {
    header("Location: specific_manual_order.php_non_registered.php?orderID=$orderID&error=product_not_found");
    exit;
}
...
if (!$row || $row['stock_qty'] < $quantity) {
    header("Location: specific_manual_order_non_registered.php?orderID=$orderID&error=insufficient_stock");
    exit;
}
$stockID = $row['stockID'];
$stmt->close();
...
if (!$stmt->execute()) {
    header("Location: specific_manual_order_non_registered.php?orderID=$orderID&error=insert_failed");
    exit;
}
...
header("Location: specific_manual_order_non_registered.php?orderID=$orderID&success=1");
exit;
?>