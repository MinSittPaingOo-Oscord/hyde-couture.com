...
if (!isset($_GET['orderItemID']) || !isset($_GET['orderID'])) {
    header("Location: specific_manual_order_non_registered.php?orderID=" . $_GET['orderID'] . "&error=missing_params");
    exit;
}
...
try {
    ...
    header("Location: specific_manual_order_non_registered.php?orderID=$orderID&success=deleted");
    exit;

} catch (Exception $e) {
    $conn->rollback();
    header("Location: specific_manual_order_non_registered.php?orderID=$orderID&error=" . $e->getMessage());
    exit;
}
