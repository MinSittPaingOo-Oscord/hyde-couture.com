...
$currentPage = "user_return_order.php";
...
$query = "SELECT * FROM orderr JOIN account ON orderr.accountID = account.accountID WHERE orderr.orderStatus = 5 $whereDate AND orderr.accountID = $userID ORDER BY orderr.orderDate DESC";
...
if ($export === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=user_return_order' . date('Ymd_His') . '.csv');
...
<head>
    ...
    <title>User Return Orders</title>
...
<body>
    <?php
        ...
        if ($result && $result->num_rows > 0) {
            ...
            echo "<div class='no-orders'>No return orders at the moment.</div>";
        }
