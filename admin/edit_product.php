...
// Fetch current data - now with photoID for deletion
$photos = $conn->query("SELECT photoID, photoName FROM photo WHERE productID = $id");
...
.photo-preview {display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:12px;margin-top:10px;}
.photo-preview img {width:100%;height:110px;object-fit:cover;border-radius:4px;border:2px solid #e0f0e0;}
...
/* NEW: Delete button for photos */
.photo-item {
    position: relative;
    display: inline-block;
}
.photo-item a.delete-btn {
    position: absolute;
    top: 6px;
    right: 6px;
        ...
        <div class="form-group">
            ...
            <div class="photo-preview">
                ...
                <div class="photo-item">
                    <img src="../image/<?= htmlspecialchars($ph['photoName']) ?>" alt="Product image">
                    <a href="delete_photo.php?photo_id=<?= $ph['photoID'] ?>&product_id=<?= $id ?>"
                       class="delete-btn"
                       onclick="return confirm('Permanently delete this photo? This cannot be undone.');"
                       title="Delete this photo">×</a>
                </div>
                <?php endwhile; ?>
            </div>
            <?php else: ?>
            <p style="color:#888;font-style:italic;margin:10px 0;">No photos uploaded yet.</p>
            <?php endif; ?>

            <label style="display:block;margin-top:20px;">Add More Photos</label>
...
// Prevent selecting both Standalone and Subcategories
