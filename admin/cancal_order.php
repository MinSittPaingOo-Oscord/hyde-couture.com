...
$query = "SELECT * FROM orderr JOIN account ON orderr.accountID = account.accountID WHERE orderr.orderStatus = 6 $whereDate ORDER BY orderr.orderDate DESC";
...
if ($export === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=cancal_orders_' . date('Ymd_His') . '.csv');

    $out = fopen('php://output', 'w');
    fputcsv($out, ['Order ID', 'Customer Name', 'Sub Total (BHATTT)', 'Order Date', 'Status']);
...
<body>
    <?php
        ...
        if ($result && $result->num_rows > 0) {
            ...
            echo "<div class='no-orders'>No cancal orders at the moment.</div>";
        }
