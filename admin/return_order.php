...
$query = "SELECT * FROM orderr JOIN account ON orderr.accountID = account.accountID WHERE orderr.orderStatus = 5 $whereDate ORDER BY orderr.orderDate DESC";
...
if ($export === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=return_orders_' . date('Ymd_His') . '.csv');
